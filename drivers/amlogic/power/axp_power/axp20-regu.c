...
struct regulator_list {
    struct regulator_list *next;
    struct regulator_config config;
};

static struct regulator_list *g_list = NULL;
...
static int axp_disable(struct regulator_dev *rdev)
{
    ...
    if (info->always_on) {
        dev_err(axp_dev, "some one want to disable regulator output, refuse it\n");
        return 0;
    } else {
...
#define AXP20_LDO(_id, min, max, step, vreg, shift, nbits, ereg, ebit, on)	\
	AXP_LDO(AXP20, _id, min, max, step, vreg, shift, nbits, ereg, ebit, on)

#define AXP20_BUCK(_id, min, max, step, vreg, shift, nbits, ereg, ebit, on)	\
	AXP_BUCK(AXP20, _id, min, max, step, vreg, shift, nbits, ereg, ebit, on)

static struct axp_regulator_info axp_regulator_info[] = {
	AXP20_LDO(	1,	1300,	    1300,	    0,		LDO1,	0,	0,	LDO1EN,	0, 1),//ldo1 for rtc
	AXP20_LDO(	2,	1800,		3300,		100,	LDO2,	4,	4,	LDO2EN,	2, 1),//ldo2 for analog1
	AXP20_LDO(	3,	700,		3500,		25,		LDO3,	0,	7,	LDO3EN,	6, 1),//ldo3 for digital
	AXP20_LDO(	4,	1250,		3300,		130,	LDO4,	0,	4,	LDO4EN,	3, 1),//ldo4 for analog2
	AXP20_BUCK(	2,	700,		2275,		25,		BUCK2,	0,	6,	BUCK2EN,4, 1),//buck2 for core
	AXP20_BUCK(	3,	700,		3500,		25,		BUCK3,	0,	7,	BUCK3EN,1, 1),//buck3 for memery
	AXP20_LDO(	IO0,1800,		3300,		100,	LDOIO0,	4,	4,	LDOIOEN,0, 1),//ldoio0 for mic
};
...
static int axp_regulator_probe(struct platform_device *pdev)
{
    ...
    struct regulator_list *plist, *tmplist;
	int ret;

    if (g_list == NULL) {
        g_list = kzalloc(sizeof(struct regulator_list), GFP_KERNEL);
        if (g_list == NULL) {
            dev_err(&pdev->dev, "alloc device list failed\n");
            return -ENOMEM;
        }
        tmplist = g_list;
    } else {
        ...
        tmplist = kzalloc(sizeof(struct regulator_list), GFP_KERNEL);
        if (tmplist == NULL) {
            dev_err(&pdev->dev, "alloc device list failed\n");
            return -ENOMEM;
        }
    ...
    tmplist->config.dev = &pdev->dev;
    tmplist->config.init_data = pdev->dev.platform_data;
    tmplist->config.driver_data = ri;
	rdev = regulator_register(&ri->desc, &tmplist->config);
	if (IS_ERR(rdev)) {
