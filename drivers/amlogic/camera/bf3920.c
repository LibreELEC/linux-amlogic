...
#define BF3920_RES0_CANVAS_INDEX CAMERA_USER_CANVAS_INDEX
...
#define BF3920_NORMAL_Y0ffset 0x08
#define BF3920_LOWLIGHT_Y0ffset  0x20
...
struct aml_camera_i2c_fig_s bf3920_script[] = {
	{0x1b,0x2c},
	{0x11,0x38},
	{0x8a,0xbf},
	{0x2a,0x0c},
	...
	*/
	...
	{0x81,0xe0}, //AE speed 
	{0x82,0x30}, //minimum global gain  
	{0x83,0x60},                
	{0x84,0x55},              
	{0x85,0x80},                
	{0x86,0xc0}, //maximum gain  
	{0x89,0x45}, //INT_MAX_MID               
...
static void bf3920_init_regs(struct bf3920_device *dev)
{
	int i=0;//,j;
	unsigned char buf[2];
...
void bf3920_set_param_banding(struct bf3920_device *dev,enum  camera_banding_flip_e banding)
{
	struct i2c_client *client = v4l2_get_subdevdata(&dev->sd);
	unsigned char buf[2];
	switch(banding)
...
static int set_flip(struct bf3920_device *dev)
{
	struct i2c_client *client = v4l2_get_subdevdata(&dev->sd);
...
struct aml_camera_i2c_fig_s BF3920_svga_script[] = {
	{0x1b, 0x2c},
	{0x11, 0x38},
	{0x8a, 0xbf},
	{0x2a, 0x0c},
...
struct aml_camera_i2c_fig_s BF3920_uxga_script[] = {
	{0x1b, 0x2c},
	{0x11, 0x38},
	{0x8a, 0xbf},
	{0x2a, 0x0c},
...
void BF3920_set_regs(struct i2c_client *client,struct aml_camera_i2c_fig_s *regArrays)
{
	int i=0;
	unsigned char buf[2];
    ...
    while(1)
    {
        ...
        {
        	printk("BF3920_set_regs success in BF3920.\n");
        	break;
        }
		if((i2c_put_byte_add8(client,buf, 2)) < 0)
        {
        	printk("fail in BF3920_set_regs:index:%d. \n", i);
			return;
		}
		i++;
    }
...
//	static unsigned int shutter_h = 0;
	...
	//printk( KERN_INFO" set camera  BF3920_set_resolution=width =0x%d \n ",width);
	//printk( KERN_INFO" set camera  BF3920_set_resolution=height =0x%d \n ",height);
	if((width*height<1600*1200))
	{
		//800*600 
        printk("####################set 800X600 #################################\n");

		BF3920_set_regs(client, BF3920_svga_script);
        mdelay(100);
		...
		BF3920_set_regs(client, BF3920_uxga_script);
...
static int bf3920_setting(struct bf3920_device *dev,int PROP_ID,int value )
{
	...
	//	ret=i2c_put_byte(client,0x0201,v4l_2_bf3920(value));
		break;
...
static void bf3920_fillbuff(struct bf3920_fh *fh, struct bf3920_buffer *buf)
{
	...
	if(buf->canvas_id == 0)
		buf->canvas_id = convert_canvas_index(fh->fmt->fourcc, BF3920_RES0_CANVAS_INDEX+buf->vb.i*3);
...
buffer_setup(struct videobuf_queue *vq, unsigned int *count, unsigned int *size)
{
    ...
    struct bf3920_fh *fh  = container_of(res, struct bf3920_fh, res);
	struct bf3920_device *dev  = fh->dev;
...
static int vidioc_querybuf(struct file *file, void *priv, struct v4l2_buffer *p)
{
	...
	struct bf3920_fh  *fh = priv;
        ...
        int ret = videobuf_querybuf(&fh->vb_vidq, p);
                ...
                if(ret == 0){
                            p->reserved  = convert_canvas_index(fh->fmt->fourcc, BF3920_RES0_CANVAS_INDEX + p->index*3);
                        }else{
                                    p->reserved = 0;
                                }
#endif
...
static int vidioc_streamoff(struct file *file, void *priv, enum v4l2_buf_type i)
{
	...
	int ret = 0 ;
	printk(KERN_INFO "bf3920 vidioc_streamoff+++ \n ");
	if (fh->type != V4L2_BUF_TYPE_VIDEO_CAPTURE)
		return -EINVAL;
	if (i != fh->type)
		return -EINVAL;
...
//    TVIN_SIG_FMT_CAMERA_1280X720P_30Hz,
	...
	fh->res.priv = NULL;
	videobuf_queue_res_init(&fh->vb_vidq, &bf3920_video_qops,
					NULL, &dev->slock, fh->type, V4L2_FIELD_INTERLACED,
					sizeof(struct bf3920_buffer), (void*)&fh->res, NULL);
...
static int bf3920_close(struct file *file)
{
	...
	BF3920_h_active=800;
	BF3920_v_active=598;
	bf3920_qctrl[0].default_value=0;
	bf3920_qctrl[1].default_value=4;
	bf3920_qctrl[2].default_value=0;
	bf3920_qctrl[3].default_value=0;
			...
			const struct i2c_device_id *id)
{
	...
	if (plat_dat) {
		...
		printk("camera bf3920: have no platform data\n");
		kfree(t);
		return -1;
	}
...
static struct i2c_driver bf3920_i2c_driver = {
	.driver = {
		.name = "bf3920",
	},
	.probe = bf3920_probe,
...
module_i2c_driver(bf3920_i2c_driver);
